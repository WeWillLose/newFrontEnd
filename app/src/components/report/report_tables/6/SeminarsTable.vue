...
const SeminarsHelper = createNamespacedHelpers('report/seminars')

export default defineComponent({
  name: 'SeminarsTable',
  setup(props,context) {
    const getColumns = SeminarsHelper.useGetters(['getColumns']).getColumns as unknown as Record < string, unknown>[];
    console.log(getColumns)
    const show_dialog = ref(false)

    let getRows =  SeminarsHelper.useGetters(['getRows']).getRows as unknown as Record < string, unknown>[];
    const getDefaultItem = SeminarsHelper.useGetters(['getDefaultItem']).getDefaultItem as unknown as Record<string, unknown>;
    const getEditedIndex = SeminarsHelper.useGetters(['getEditedIndex']).getEditedIndex as unknown as number;
    const getEditedItem = SeminarsHelper.useGetters(['getEditedItem']).getEditedItem as unknown as Record<string, unknown>;
  ...
  methods: {
    addRowHandler() {
      this.$store.commit('report/seminars/addRow', {'editedItem':this.getEditedItem,'editedIndex':this.getEditedIndex})
      this.close()
    },
    deleteItem(item: Record<string, unknown>) {
      confirm('Вы уверены что хотите удалить строку') && this.$store.commit('report/seminars/deleteItem',item)
    },
    editItem(item: Record<string, unknown>) {
      this.$store.commit('report/seminars/editItem',item)
      this.show_dialog = true;
    },
    close () {
      this.show_dialog = false
      setTimeout(() => {this.$store.commit('report/seminars/setDefault')}, 300)
    }
  }
