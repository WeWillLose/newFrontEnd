<template>
  <div class="q-pa-sm q-gutter-sm">
    <q-table title="4.2 Наличие предметного кружка, эффективность его работы" :hide-no-data="true" :data="getRows" :columns="getColumns"
      ...
      <template v-slot:body="props">
        <q-tr :props="props">
          <q-td key="group" :props="props">
            <q-input type="text" v-model="props.row.group" dense autofocus></q-input>
          </q-td>
          <q-td key="students" :props="props">
            <q-input type="number" v-model="props.row.students" dense autofocus></q-input>
          </q-td>
          <q-td key="lessons" :props="props">
            <q-input type="number" v-model="props.row.lessons" dense autofocus></q-input>
          </q-td>
...
const CircleHelper = createNamespacedHelpers('report/circle')

export default defineComponent({
  name: 'CircleTable',
  setup(props,context) {
    const getColumns = CircleHelper.useGetters(['getColumns']).getColumns as unknown as Record < string, unknown>[];
    console.log(getColumns)
    const show_dialog = ref(false)

    let getRows =  CircleHelper.useGetters(['getRows']).getRows as unknown as Record < string, unknown>[];
    const getDefaultItem = CircleHelper.useGetters(['getDefaultItem']).getDefaultItem as unknown as Record<string, unknown>;
    const getEditedIndex = CircleHelper.useGetters(['getEditedIndex']).getEditedIndex as unknown as number;
    const getEditedItem = CircleHelper.useGetters(['getEditedItem']).getEditedItem as unknown as Record<string, unknown>;
  ...
  methods: {
    addRowHandler() {
      this.$store.commit('report/circle/addRow', {'editedItem':this.getEditedItem,'editedIndex':this.getEditedIndex})
      this.close()
    },
    deleteItem(item: Record<string, unknown>) {
      confirm('Вы уверены что хотите удалить строку') && this.$store.commit('report/circle/deleteItem',item)
    },
    editItem(item: Record<string, unknown>) {
      this.$store.commit('report/circle/editItem',item)
      this.show_dialog = true;
    },
    close () {
      this.show_dialog = false
      setTimeout(() => {this.$store.commit('report/circle/setDefault')}, 300)
    }
  }
