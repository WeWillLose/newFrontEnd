<template>
  <div class="q-pa-sm q-gutter-sm">
    <q-table title="4.4 Проведение мероприятий (отделение, колледж)" :hide-no-data="true" :data="getRows" :columns="getColumns"
        ...
        <q-tr :props="props">
          ...
          <q-td key="score" :props="props">
            <q-input type="number" v-model="props.row.score"
                     :rules="[ruleApi.withoutSpaces(),ruleApi.numberBetweenNotStrictScore(0,1.5)]" dense autofocus></q-input>
          </q-td>
...
const EventsHelper = createNamespacedHelpers('report/events')

export default defineComponent({
  name: 'EventsTable',
  setup(props,context) {
    const getColumns = EventsHelper.useGetters(['getColumns']).getColumns as unknown as Record < string, unknown>[];
    console.log(getColumns)
    const show_dialog = ref(false)

    let getRows =  EventsHelper.useGetters(['getRows']).getRows as unknown as Record < string, unknown>[];
    const getDefaultItem = EventsHelper.useGetters(['getDefaultItem']).getDefaultItem as unknown as Record<string, unknown>;
    const getEditedIndex = EventsHelper.useGetters(['getEditedIndex']).getEditedIndex as unknown as number;
    const getEditedItem = EventsHelper.useGetters(['getEditedItem']).getEditedItem as unknown as Record<string, unknown>;
  ...
  methods: {
    addRowHandler() {
      this.$store.commit('report/events/addRow', {'editedItem':this.getEditedItem,'editedIndex':this.getEditedIndex})
      this.close()
    },
    deleteItem(item: Record<string, unknown>) {
      confirm('Вы уверены что хотите удалить строку') && this.$store.commit('report/events/deleteItem',item)
    },
    editItem(item: Record<string, unknown>) {
      this.$store.commit('report/events/editItem',item)
      this.show_dialog = true;
    },
    close () {
      this.show_dialog = false
      setTimeout(() => {this.$store.commit('report/events/setDefault')}, 300)
    }
  }
